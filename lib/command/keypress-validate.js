...
function Validate() {
  return async function keyPress(state, press) {
    if (press.key.name === "return") {
      if (!state.commandLine().hasAction) {
        state.returnCommand(false);
        state.header(
          chalk`{yellow "${state.prompt.command.text.trim()}" is not a complete command}`
        );
      }
      const missing = _get(state.commandLine(), "input.missing", []);
      if (missing.length) {
        const args = _map(missing, arg => arg.name);
        const { name, description } = missing[0];
        state.returnCommand(false);
        state.header(
          chalk`{yellow Missing Args: ${args.join(
            " "
