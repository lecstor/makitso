 ...
 * @property {Object} prompt - an enquirer question definition
...
function Context(args = {}) {
  const { schema, stores, prompt } = args;
  return {
    /**
    ...
    get: async function(prop) {
      ...
      if (value && !(meta.valueAsDefault || meta.when === "always")) {
        return value;
      }

      const variant = (meta.variant !== "default" ? meta.variant : false) || "";
      const { header, prompt: mPrompt, footer } = meta.ask;
      const { valueAsDefault, default: mDefault, secret } = meta.ask;
      const thisPrompt = {
        ...
        default: value && valueAsDefault ? value : mDefault,
        secret
      };
