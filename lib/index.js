 ...
 * @param {Object} args.enquirer - instance on enquirer
 * @returns {Object} the updated pluginSet
 */
function registerPlugin(args) {
  const { plugin, enquirer } = args;
  let { pluginSet } = args;
  ...
  if (plugin.prompts) {
    _forEach(plugin.prompts, (ePlugin, type) => {
      enquirer.register(type, ePlugin);
    });
  }
  return pluginSet;
}
...
function Makitso(args) {
  ...
  return {
    ...
    start() {
      ...
      debug({ commands });
      const context = Context({ schema, stores, enquirer: this.enquirer });
