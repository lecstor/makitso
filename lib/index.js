 ...
 * @param {String|Prompt} [args.prompt="Makitso>"] - prompt text or makitso-prompt instance
...
function Makitso(args = {}) {
  ...
  return Promise.all(_flatten(plugins))
    .then(pluginList => {
    ...
    .then(pluginSet => {
      ...
      const context = Context({ schema, stores, commands, prompt });
      return command({ context, commands });
    });
}
